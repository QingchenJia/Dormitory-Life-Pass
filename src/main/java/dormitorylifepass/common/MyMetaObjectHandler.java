...
public class MyMetaObjectHandler implements MetaObjectHandler {
    ...
    public void insertFill(MetaObject metaObject) {
        ...
        setFieldIfPresent(metaObject, "createUser", request.getSession().getAttribute("currentUser"));

        // 同时填充 updateTime 和 updateUser，因为在插入时也是第一次更新
        setFieldIfPresent(metaObject, "updateTime", LocalDateTime.now());
        setFieldIfPresent(metaObject, "updateUser", request.getSession().getAttribute("currentUser"));
    }
